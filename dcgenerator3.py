# dc generator separately excited - shunt
...
def dcgenerator3(difficulty_level):

    # Symbols
    I_LOAD = Symbol('I_LOAD')
    V_f = Symbol('V_f')
    I_f = Symbol('I_f')
    R_f = Symbol('R_f')
    V_t = Symbol('V_t')
    ...
    P_load = Symbol('P_load')
    P_generated = Symbol('P_generated')

    # Equations
    e0 = ([I_LOAD, I_a, I_f], Eq(I_LOAD, I_a+I_f), 'e0')
    e1 = ([V_t, I_f, R_f], Eq(V_t, I_f*R_f), 'e1')
    e2 = ([E_a, I_a, R_a, V_t], Eq(E_a, (I_a*R_a) + V_t), 'e2')
    e3 = ([E_a, phi, N, z, P, a], Eq(E_a, (phi*N*z*P)/(60*a)), 'e3')
    e4 = ([P_generated, E_a, I_a], Eq(P_generated, E_a*I_a), 'e4')
    e5 = ([P_load, V_t, I_LOAD], Eq(P_load, V_t*I_LOAD), 'e5')
    e7 = ([eff, P_generated, P_load], Eq(eff, (P_load/P_generated)*100), 'e7')
    e8 = ([T_e, P_generated, omega_m], Eq(T_e, (P_generated/omega_m)), 'e8')
    e9 = ([omega_m, N], Eq(omega_m, (2*pi*N)/60), 'e9')
    e10 = ([E_a, I_a, T_e, omega_m], Eq(E_a, (T_e*omega_m)/I_a), 'e10')
    e11 = ([T_e, K_a, phi, I_a], Eq(T_e, K_a*phi*I_a), 'e11')
    e12 = ([E_a, K_a, phi, omega_m], Eq(E_a, K_a*phi*omega_m), 'e12')
    e13 = ([K_a, z, P, a], Eq(K_a, (z*P)/(2*pi*a)), 'e13')

    list_of_equations = [e1, e2, e3, e4, e5, e0, e7, e8, e9, e10, e11, e12, e13]

    dict_of_equations = {'e1': e1, 'e2': e2, 'e3': e3, 'e4': e4, 'e5': e5,
                         'e0': e0, 'e7': e7, 'e8': e8, 'e9': e9, 'e10': e10,
                         'e11': e11, 'e12': e12, 'e13': e13}

    list_of_variables = [V_f, I_f, R_f, V_t, E_a, I_a, R_a, N, phi, z, P
                         , a, T_e, omega_m, eff, P_generated, P_load, K_a]

    always_known = [a, P]

    dict_of_var_names = {V_f: 'Field Voltage',
                         I_f: 'Field Current',
                         I_LOAD: 'Load Current',
                         ...
                         P_load: 'Power Delivered to Load',
                         P_generated: 'Power Generated',
    ...
    dict_of_values1 = {
                     ...
                     N: sample([1150], 1)[0],
                     phi: nm.round(uniform(0.0108, 0.0109), decimals=4),
                     z: sample([1200], 1)[0],
                     P: 4,
                     a: 4,
                     T_e: randint(205, 210),
                     ...
                     eff: randint(91, 92),
                     P_load: randint(22500, 23500),
                     P_generated: randint(24500, 25500),
                     }
    ...
    dict_of_values2 = {
                     ...
                     R_a: nm.round(uniform(0.2, 0.17), decimals=2),
                     N: sample([1150], 1)[0],
                     phi: nm.round(uniform(0.005, 0.006), decimals=4),
                     z: sample([1200], 1)[0],
                     P: 4,
                     a: 2,
                     T_e: randint(205, 210),
    ...
    # question
    # =========
    question_html = '<img  class="img-responsive center-block" src="/static/img/dcgenerator3.svg" /><br>'

    question_html += '<p>A Shunt ' \
                     'DC Generator is operating under the ' \
